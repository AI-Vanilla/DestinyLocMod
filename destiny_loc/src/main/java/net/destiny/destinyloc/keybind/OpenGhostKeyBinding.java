...
package net.destiny.destinyloc.keybind;
...
import net.destiny.destinyloc.procedures.Push_g_keyProcedure;
...
public class OpenGhostKeyBinding extends DestinyLocModElements.ModElement {
	@OnlyIn(Dist.CLIENT)
	private KeyBinding keys;
	public OpenGhostKeyBinding(DestinyLocModElements instance) {
		super(instance, 94);
	...
	public void initElements() {
		keys = new KeyBinding("key.destiny_loc.open_ghost", GLFW.GLFW_KEY_G, "key.categories.ui");
		...
		if (Minecraft.getInstance().currentScreen == null) {
			if (event.getKey() == keys.getKey().getKeyCode()) {
				if (event.getAction() == GLFW.GLFW_PRESS) {
					DestinyLocMod.PACKET_HANDLER.sendToServer(new KeyBindingPressedMessage(0, 0));
		...
		if (type == 0) {
			{
				...
				Push_g_keyProcedure.executeProcedure($_dependencies);
			}
		}
	}
}
