...
@OnlyIn(Dist.CLIENT)
public class GhostmapeuropaGuiWindow extends ContainerScreen<GhostmapeuropaGui.GuiContainerMod> {
	private World world;
	private int x, y, z;
	...
	public void init(Minecraft minecraft, int width, int height) {
		...
		this.addButton(new Button(this.guiLeft + 72, this.guiTop + 3, 50, 20, new StringTextComponent("Quest"), e -> {
			if (true) {
				DestinyLocMod.PACKET_HANDLER.sendToServer(new GhostmapeuropaGui.ButtonPressedMessage(0, x, y, z));
				GhostmapeuropaGui.handleButtonAction(entity, 0, x, y, z);
			}
		}));
		this.addButton(new Button(this.guiLeft + 305, this.guiTop + 3, 55, 20, new StringTextComponent("Season"), e -> {
			if (true) {
				DestinyLocMod.PACKET_HANDLER.sendToServer(new GhostmapeuropaGui.ButtonPressedMessage(1, x, y, z));
				GhostmapeuropaGui.handleButtonAction(entity, 1, x, y, z);
			}
		}));
		this.addButton(new Button(this.guiLeft + 225, this.guiTop + 3, 65, 20, new StringTextComponent("Location"), e -> {
			if (true) {
				DestinyLocMod.PACKET_HANDLER.sendToServer(new GhostmapeuropaGui.ButtonPressedMessage(2, x, y, z));
				GhostmapeuropaGui.handleButtonAction(entity, 2, x, y, z);
			}
		}));
		this.addButton(new Button(this.guiLeft + 176, this.guiTop + 90, 45, 20, new StringTextComponent("GATE"), e -> {
			if (true) {
				DestinyLocMod.PACKET_HANDLER.sendToServer(new GhostmapeuropaGui.ButtonPressedMessage(3, x, y, z));
				GhostmapeuropaGui.handleButtonAction(entity, 3, x, y, z);
			}
		}));
		this.addButton(new Button(this.guiLeft + 262, this.guiTop + 81, 105, 20, new StringTextComponent("DEEP STONE CRYPT"), e -> {
			if (true) {
				DestinyLocMod.PACKET_HANDLER.sendToServer(new GhostmapeuropaGui.ButtonPressedMessage(4, x, y, z));
				GhostmapeuropaGui.handleButtonAction(entity, 4, x, y, z);
			}
		}));
	}
}
