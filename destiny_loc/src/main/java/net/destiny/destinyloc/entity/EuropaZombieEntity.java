...
import net.destiny.destinyloc.entity.renderer.EuropaZombieRenderer;
import net.destiny.destinyloc.DestinyLocModElements;

@DestinyLocModElements.ModElement.Tag
public class EuropaZombieEntity extends DestinyLocModElements.ModElement {
	public static EntityType entity = (EntityType.Builder.<CustomEntity>create(CustomEntity::new, EntityClassification.MONSTER)
			...
			.size(0.6f, 1.8f)).build("europa_zombie").setRegistryName("europa_zombie");

	public EuropaZombieEntity(DestinyLocModElements instance) {
		super(instance, 40);
		FMLJavaModLoadingContext.get().getModEventBus().register(new EuropaZombieRenderer.ModelRegisterHandler());
	...
	public void initElements() {
		...
		elements.items.add(() -> new SpawnEggItem(entity, -1, -16764109, new Item.Properties().group(DestinyLoCItemGroup.tab))
				.setRegistryName("europa_zombie_spawn_egg"));
	}

	@SubscribeEvent
	public void addFeatureToBiomes(BiomeLoadingEvent event) {
		boolean biomeCriteria = false;
		if (new ResourceLocation("destiny_loc:europa").equals(event.getName()))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
	...
	private static class EntityAttributesRegisterHandler {
		...
		public void onEntityAttributeCreation(EntityAttributeCreationEvent event) {
			...
			ammma = ammma.createMutableAttribute(Attributes.MAX_HEALTH, 87);
